...
export interface ReceiptData {
  ...
  is_test?: boolean
  /**
   * Если `true` — это чек возврата (refund).
   * Печатается с шапкой «ВОЗВРАТ ТОВАРА» вместо «КАССОВЫЙ ЧЕК»,
   * + подзаголовок со ссылкой на оригинальный чек (см. original_receipt_ref).
   * Денежные суммы остаются положительными (Communicator/ОФД сами учитывают
   * знак refund), но контекст печати «деньги ВЫДАНЫ покупателю».
   */
  is_refund?: boolean
  /**
   * Ссылка на оригинал для refund-чека: "ABC123 от 14.05.2026 16:12".
   * Печатается под шапкой «ВОЗВРАТ ТОВАРА». Игнорируется если is_refund=false.
   */
