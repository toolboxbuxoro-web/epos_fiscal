...
export const SettingKey = {
  ...
  /**
   * Потолок штук одного товара в строке чека. По умолчанию 20.
   *
   * Ограничение мягкое: если план с ним не сходится, подбор повторяется без
   * него — блокировать продажу ради правдоподобия чека нельзя.
   */
  MaxQtyPerLine: 'matcher.max_qty_per_line',
  /**
...
export type MsReceiptStatus =
  ...
  | 'not_required'
  /**
   * Чек в валюте, которой касса не умеет фискализировать (или у аккаунта
   * МойСклад сменилась базовая валюта — тогда суммы приходят в других
   * минорных единицах). Фискализация остановлена намеренно: лучше внятный
   * отказ, чем чек, ушедший в ОФД с суммой, заниженной в тысячи раз.
   */
  | 'currency_mismatch'
