...
export interface FiscalDriveItem {
  ...
  SPIC: string
  /**
   * Единица измерения — идентификатор из справочника Tasnif, привязанный к
   * конкретному ИКПУ (как `PackageCode`), а НЕ короткий ОКЕИ-код.
   * Примеры из официальной спецификации FiscalDriveService: 244272402,
   * 142235374; у Payme — 241092. Формат переменной длины.
   *
   * По спецификации поле **необязательное** (`Units | uint64 | No`).
   *
   * ⚠️ Мы раньше слали сюда 796 — российский ОКЕИ «штука». ОФД принимал чек,
   * но в налоговом отчёте единица измерения выходила неверной у ВСЕХ чеков
   * магазина на FiscalDrive. У магазинов на EPOS Communicator поля нет в
   * протоколе вообще, и отчёты у них корректны — то есть отсутствие значения
   * лучше, чем неверное. Поэтому шлём только когда есть настоящее значение
   * из справочника; иначе поле опускаем.
   */
  Units?: number
  PackageCode: string
