      ...
      <div className="overflow-x-auto rounded-lg border border-border bg-surface">
        <table className="min-w-full divide-y divide-border text-sm">
          ...
          <tbody className="divide-y divide-border">
            {rows.length === 0 ? (
              <tr>
                <td className="px-3 py-8 text-center text-sm text-ink-muted" colSpan={4}>
                  Логов пока нет.
                </td>
              </tr>
            ) : (
              rows.map((r) => (
                <>
                  ...
                  >
                    ...
                    <Td className="text-xs text-ink-muted">{r.source}</Td>
                    <Td className="font-medium text-ink">{r.message}</Td>
                  </tr>
                  {expandedId === r.id && r.details && (
                    <tr key={`${r.id}-d`}>
                      <td colSpan={4} className="bg-canvas px-4 py-2">
...
const LEVEL_STYLES: Record<LogLevel, string> = {
  debug: 'bg-surface-hover text-ink-muted',
...
function Th({ children, className = '' }: { children: React.ReactNode; className?: string }) {
  return (
    <th className={`px-3 py-2 text-left text-xs font-medium text-ink-muted ${className}`}>
      {children}
    </th>
  )
}
