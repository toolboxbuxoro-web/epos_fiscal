...
Себестоимость с НДС: `unit_price × (1 + vat/100)` × quantity — функция
`costWithVat`. Голая себестоимость без наценки.

**Минимальная наценка (floor) = себестоимость × (1 + 5%)** — функция
`priceFloorTiyin`, константа `MIN_MARKUP_PERCENT = 5` (хардкод). Это
нижняя граница цены при подборе: товар не продаётся ниже «себестоимость
+ 5%». Применяется в:
- `distributeDiscount` — скидка не режет ниже floor (раньше резала до
  голой себестоимости = 0% маржи)
- `holistic` фаза-3 (discount при перенаборе) + global floor check
- `holistic` фаза-1 greedy (скипает товар если floor > sellingPrice)
- **post-match warning** в `buildMatch`: если `effectivePrice < floor`
  (бывает у linked-ms/price-bucket где цена = `pos.totalTiyin`, и клиент
  заплатил меньше чем себестоимость+5%) → жёлтый warning кассиру
  «продаётся ниже минимальной цены». НЕ блокируем — цену чека МС менять
  нельзя, клиент уже заплатил. Кассир может заменить товар через ручную
  сборку.

При наценке 10% (дефолт) calculated selling всегда выше floor (+5%),
...
| Магазин молча перестал синхронизировать продажи (0 чеков на сервере при исправной фискализации) | Ни на одном исходящем `fetch` не было таймаута. `fetch` не сдаётся никогда, а у флашера продаж guard `inFlight`: зависший POST не завершал промис, guard не снимался, каждый следующий тик возвращал ту же зависшую промису | `fetchWithTimeout` (`src/lib/http.ts`) на все фоновые вызовы + сторож на `inFlight` (10 мин). Плагин `@tauri-apps/plugin-http` поддерживает `AbortSignal` — по abort зовёт `fetch_cancel`, запрос реально обрывается |
| Поломка фоновой синхронизации не видна снаружи неделями | Ошибки отправки писались через `log.warn`, а warn остаётся ЛОКАЛЬНО — в телеметрию уходит только `level='error'`. Отказ выглядел как исправная работа | Проверка здоровья очереди раз в 30 мин (старейший неотправленный чек > суток → `log.error`); затяжной сбой (5 попыток) эскалируется warn → error; ненастроенный сервер больше не немой `return` |
| Один плохой чек навсегда блокировал очередь продаж магазина | `ingestSales` на сервере обрабатывал всю пачку из 20 чеков ОДНОЙ транзакцией: любая ошибка → ROLLBACK всей пачки → 500 → клиент повторяет ТУ ЖЕ пачку вечно | SAVEPOINT на каждый чек (как в `bulkImport`): плохой откатывается один, причина возвращается в `failed[]`, клиент помечает такой чек synced и пишет `log.error` |
| Общий таймаут сломал бы полную выгрузку приходов | 20 сек ставились на все вызовы склада, но `listItems` тянет страницы по 1000 позиций (сотни КБ) — оборванная выгрузка возвращает stale-кэш и биение подбора об остатки | Таймаут задаётся на запрос: короткие вызовы 20 сек, `listItems` — 60 (`BULK_TIMEOUT_MS`) |
...
## Текущее состояние (на 2026-08-25)

### Версии

- **Последний released tag:** `v0.11.25` (25.08.2026) — GitHub Actions строит Win+Mac, авто-апдейт идёт на все магазины
- **Текущая версия:** `0.11.25` (в package.json + tauri.conf.json + `src/lib/app-version.ts::APP_VERSION` — все 3 синхронны)
- **Тесты:** vitest, `npm test`. 426 кейсов (holistic, partial-refund, preflight, stale-handler, price-floor, rounding на реальных данных БД, fiscal-drive-client, **http-таймауты**). Бэкенд mytoolbox: `npm test` в `backend/`, 352 кейса (node --test), включая изоляцию плохого чека при приёме продаж

### ⚠️ Таймауты: где есть, где нет

`fetchWithTimeout` (`src/lib/http.ts`) стоит на ФОНОВЫХ путях — продажи (60 сек),
телеметрия (20), клиент склада (20, а `listItems` 60). Именно они могут отказать
молча, и один такой отказ стоил магазину Хонабод трёх месяцев истории продаж.

**Таймаутов пока НЕТ** у опроса МойСклад (`moysklad/client.ts`), вызовов
Communicator (`epos/jsonrpc-client.ts`, `epos/fiscal-drive-client.ts`) и входа
(`inventory/login.ts`). Это осознанно отложено, а не забыто: там зависание
видно сразу — кассир смотрит на висящий спиннер и сообщает. Проставить лимиты
там всё равно стоит (понятная ошибка вместо вечного ожидания), но подбирать
значения нужно аккуратно: фискализация законно занимает 30–60 сек, пока
Communicator общается с ОФД.

**`sse.ts` таймаутить НЕЛЬЗЯ** — это долгоживущее соединение по замыслу.
